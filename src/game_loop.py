...
from minimax import MinimaxExtended
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if game_state.moves_count == 0:
            ...
            task_args = [(game_state.deepcopy(), game_state.player_turn, 0, -inf, inf) for _ in game_state.get_available_moves()]
            with Pool(
                processes = cpu_count(),
                initializer = MinimaxExtended.initialize_max_depth,
                initargs = (depth, )
            ) as pool:
                results = pool.starmap(MinimaxExtended.minimax, task_args)
            
            move_idx, _ = max(results, key=lambda x: x[1])
